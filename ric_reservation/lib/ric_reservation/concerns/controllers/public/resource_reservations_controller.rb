			...
			module Public
				module ResourceReservationsController extend ActiveSupport::Concern
					...
					def create
						...
						if @reservation.save
							respond_to do |format|
								format.html { redirect_to ric_reservation_public.resource_reservation_path(@reservation), notice: I18n.t("activerecord.notices.models.#{RicReservation.reservation_model.model_name.i18n_key}.create") }
					...
					def update
						if @reservation.update(reservation_params)
							respond_to do |format|
								format.html { redirect_to ric_reservation_public.resource_reservation_path(@reservation), notice: I18n.t("activerecord.notices.models.#{RicReservation.reservation_model.model_name.i18n_key}.update") }
								format.json { render json: @reservation.id }
							end
						else
							respond_to do |format|
					...
					def destroy
						@reservation.destroy
						respond_to do |format|
							format.html { redirect_to ric_reservation_public.timetable_path(@reservation.resource_id), notice: I18n.t("activerecord.notices.models.#{RicReservation.reservation_model.model_name.i18n_key}.destroy") }
							format.json { render json: @reservation.id }
						end
					end
				...
				private
					...
					def reservation_params
						params.require(:reservation).permit(:resource_id, :name, :schedule_date, :time_from, :time_to, :color)
					end
